...
def parse_xml(xml_path: Union[str, pathlib.Path]):
    ...
    # extracting figures and captions and in-text references to figures
    ...
    for fig in figs:
        ...
        if fig.graphic:
            ...
            print(f'ERROR: no graphic or media is parsed from xml figs, skipping {xml_path}')
            continue

        if file_extension not in ['mov', 'jpg', 'dcr', 'avi', 'mpeg', 'pdf', 'mp4', 'docx', 'png']:
            # raise RuntimeError(f'{xml_path} contains media we dont know before: {media_name}, {media_url}')
            print(f'WARNING: {xml_path} contains media we dont know before: {media_name}, {media_url}')
        ...
        # extract in-text refs if they exist
        intext_refs = soup.find_all(name="xref", attrs={"ref-type": "fig", "rid": media_id})
        intext_refs_context = []
        for ref in intext_refs:
            context = "".join(ref.parent.stripped_strings)
            intext_refs_context.append(context)

        item_info.append({
            'PMC_ID': PMC_ID,
            'media_id': media_id,  # media_id could represent image or video. [image: pbio-0020008-g002; video: pbio-0020008-v001]
            'caption': caption,
            'intext_refs': intext_refs_context,
            'media_url': media_url,

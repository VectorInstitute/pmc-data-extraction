...
from openpmcvl.process.dataset_det_align import Fig_Separation_Dataset, fig_separation_collate
...
def load_dataset(eval_file: str, img_root: str, batch_size: int, num_workers: int) -> DataLoader:
    """
    ...
    return DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, collate_fn=fig_separation_collate)


def load_model(checkpoint_path: str, device: torch.device) -> FigCap_Former:
    """
...
def process_detections(det_boxes: torch.Tensor, det_scores: np.ndarray, nms_threshold: float) -> Tuple[List[List[float]], List[float]]:
    """
    ...
    while order.size > 0:
        ...
        if order.size == 1:
            break
        iou_with_left = find_jaccard_overlap(box_cxcywh_to_xyxy(det_boxes[index]), box_cxcywh_to_xyxy(det_boxes[order[:-1]])).squeeze().numpy()
        left = np.where(iou_with_left < nms_threshold)
        order = order[left]
    return picked_bboxes, picked_scores


def separate_subfigures(model: FigCap_Former, loader: DataLoader, save_path: str, rcd_file: str, score_threshold: float, nms_threshold: float, device: torch.device) -> None:
    """
    Separates compound figures into subfigures.
        ...
        try:
            for batch in tqdm(loader, desc='Separating subfigures', total=len(loader)):
                ...
                for i in range(image.shape[0]):
                    ...
                    det_scores = cpu_output_det_class.squeeze()[i, filter_mask[i,:]].numpy()
                    ...
                    picked_bboxes, picked_scores = process_detections(det_boxes, det_scores, nms_threshold)
                    
                    for bbox, score in zip(picked_bboxes, picked_scores):
                        try:
                            subfig_path = f'{save_path}/{img_id}_{subfig_count}.jpg'
                            cx, cy, w, h = bbox
                            x1, x2 = [round((cx - w/2)*image.shape[3]*scale), round((cx + w/2)*image.shape[3]*scale)]
                            y1, y2 = [round((cy - h/2)*image.shape[2]*scale), round((cy + h/2)*image.shape[2]*scale)]
                            
                            x1, x2 = [max(0, min(x, original_w-1)) for x in [x1, x2]]
                            y1, y2 = [max(0, min(y, original_h-1)) for y in [y1, y2]]
                            
                            subfig = unpadded_image[:, y1:y2, x1:x2].to(torch.device('cpu'))
                            ...
                            print(f'Crop Error: [x1 x2 y1 y2]:[{x1} {x2} {y1} {y2}], w:{original_w}, h:{original_h}')
...
def main(args: argparse.Namespace) -> None:
    ...
    dataloader = load_dataset(args.eval_file, args.img_root, args.batch_size, args.num_workers)
    separate_subfigures(model, dataloader, args.save_path, args.rcd_file, args.score_threshold, args.nms_threshold, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_file', type=str, required=True, help='Path to evaluation dataset file')
    parser.add_argument('--img_root', type=str, required=True, help='Root path for figures')
    parser.add_argument('--save_path', type=str, default='./Separation', help='Path to save separated subfigures')
    parser.add_argument('--rcd_file', type=str, default='./Separation/separation.jsonl', help='File to record separation results')
    parser.add_argument('--score_threshold', type=float, default=0.75, help='Confidence score threshold for detections')
